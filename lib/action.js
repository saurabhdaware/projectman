...
async function openProject(projectName) {
    ...
    try {
        if (/^https?:\/\//.test(selectedProject.path)) {
            const {stderr} = await openURL(selectedProject.path)
        } else {
        ...
        console.log(`${color.boldGreen(">>>")} Opening ${selectedProject.name} ${color.green("✔")}`);
...
async function getProjectPath(projectName) {
    ...
    const hint = settings.projects.map((project,index) => (index+1) + '.' + project.name).join('\n');
    ...
    const {index} = await prompts([question],{onCancel})
    console.log(settings.projects[(index - 1)].path);
