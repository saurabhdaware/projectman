...
async function selectProject(projectName, action) {
    ...
    if (!projectName) {
        ...
        ({selectedProject} = await prompts(questions));
        if(!selectedProject){
            console.error("Could not find project. Terminating process.");
            process.exit();
            return;
        }
...
async function setEditor(command, cmdObj = undefined) {
    ...
    if (!command) {
        ...
        const {selectedEditor} = await prompts(questions);
        if (selectedEditor == 'other') {
            ...
            const {command} = await prompts([question])
